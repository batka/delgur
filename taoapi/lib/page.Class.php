...
class PageClass
{
	...
	{	
		...
		if($this->myde_count < 10240)
			$this -> myde_page_count  = ceil($this -> myde_count/$this -> myde_size);
		else
			$this -> myde_page_count  = ceil(10240 / $this->myde_size);
	...
	function myde_write($id='page') //输出
	{
		...
		$str .= "  </ul>\n";
		$str .= "<div class=\"page_clear\"></div>\n";
		$str .= "<div style='text-align:right;margin-top:5px'>温馨提示：淘宝最多只能检索到10240个商品，所以当每页显示10条记录时，最多能看到 1024页</div>";
		$str .= "<div class=\"page_clear\"></div>\n</div>";
