...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		/*
		foreach ($this->session->data['cart'] as $item => $vv)
		{
			list($pid, $optionsss) = explode(':', $item);
			print_r(unserialize(base64_decode($optionsss)));
			echo "\n";
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
        		...
        		$this->data['products'][] = array(
					...
					'name'     => $product['name'],
          			'model'    => $product['model'],
          			'option'   => $product['option'],//$option_data,
	...
	public function update() {
		...
		if (isset($this->request->post['product_id'])) {
			...
			if ($product_info) {
				...
				{
					$option = $this->request->post['option'];
					$Skus = $this->getSkus($this->request->post['product_id']);
					$options = unserialize($product_info['options']);
					...
					foreach($option as $key => $value)
					{
						if ($value == '')
						{
							$json['error']['warning'][] = sprintf($this->language->get('error_required'), $options['options'][$key]);;
						}
						else
						{
							$sku1 .= $key.':'.$value . ';';
					...
					$sku1 = substr($sku1, 0, -1);
					$sku2 = substr($sku2, 0, -1);
					
					$sku = isset($Skus[$sku1]) ? $Skus[$sku1] : '';
					$sku = ($sku == '' && isset($Skus[$sku2])) ? $Skus[$sku2] : '';
				}
				else
				{
					$option = array();	
		...
		$json['total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $total_data[0]['text']);
	...
	function getSkus($id)
	{
		...
		if (file_exists($file))
		{
			$sSkus = file_get_contents($file);
			
			if ($sSkus)
			{
				return unserialize($sSkus);
			}
		}
		return '';
	}
