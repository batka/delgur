...
class ModelAccountOrder extends Model {
	...
	public function updateWesternUnionPin($order_id, $western_union_name, $western_union_pin) {
		$order_query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND order_status_id > '0'");
		if ($order_query->num_rows) {
			$order_id = $order_query->row['order_id'];
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET western_union_name = '" . $western_union_name . "', western_union_pin = '" . $western_union_pin . "' WHERE order_id = '" . (int)$order_id . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND order_status_id > '0'");
			return true;
		}
		return false;
	}
